function [u_s] = solver_u_robin(visc_s, visc, AGlen_s, para)
% Inputs:
% visc_s [Pa a]
% visc [Pa a]
% AGlen_s [Pa-3 a-1]
...
elseif strcmpi(type_BBC, 'CFlaw_polyT')
    ...
    Lb_CFL = 4*dzeta.*AGlen_s(1,:).*H_s.*(Cb.^n).*(Neff_lst.^n).*m_max ./...
        (m_max.*ub_s + (Cb.^n).*lambda_max.*AGlen_s(1,:).*(Neff_lst.^n)) + 0.01;
...
elseif strcmpi(type_BBC, 'CFlaw_isoT')
    ...
    % Flowers (2011); 0.01 here is a regularization term.
    Lb_CFL = 4*dzeta.*AGlen_s(1,:).*H_s.*(Cb.^n).*(Neff_lst.^n).*m_max ./...
        (m_max.*ub_s + (Cb.^n).*lambda_max.*AGlen_s(1,:).*(Neff_lst.^n)) + 0.01;
