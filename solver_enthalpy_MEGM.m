...
function [E, T, omega, Kappa_vs, CTS, thk_TEMP, m_basal, qw_TEMP, qw_TEMP_darcy] =...
    solver_enthalpy_MEGM(u, u_s, w, w_vs, strainHeat, frictionHeat, dt, Esbc, Eini, para)
global M N dx dzeta zeta H dzetadx iTimeStep...
    At_E At_Kappa_vs At_qw_TEMP_darcy At_isTEMP At_thk_TEMP At_thk_w
...
k0 = para.k0;
eta_w = para.eta_w;
alpha_hewitt = para.alpha_hewitt; % % exponent in Hewitt-Schoof model
...
if iTimeStep==1
    ...
    qw_TEMP_darcy_lst = zeros(N,M);
else
    is_trans = 1;
    Enm1 = At_E(:,:,iTimeStep-1);
    Kappa_vs_lst = At_Kappa_vs(:,:,iTimeStep-1);
    qw_TEMP_darcy_lst = At_qw_TEMP_darcy(:,:,iTimeStep-1);
end
...
for i=1:M
    ...
    for j = 2:N-1
        if M==1 % ice slab case (no horizontal advection and diffusion)
            RT(j) = is_trans*Enm1(j,i)/(dt) + strainHeat(j,i)/rhoi -...
                ...
                qw_TEMP_darcy_lst(j-1,i))/(H(i)*dzeta);
        else
            if i==1
                RT(j) = is_trans*Enm1(j,i)/(dt) + strainHeat(j,i)/rhoi;
            else
    ...
    if isempty(jcts)      %% cold
        ...
        conudctHeat = para.kc*(T(2,i)-T(1,i))/(H(i)*dzeta);
        m_basal(i) = (para.Qgeo + conudctHeat + frictionHeat(i))/(rhow*Lw)*SPY; % basal melt [m a-1]
...
%         end
    ...
    qw_TEMP_darcy(:,i) = -k0*omega(2:end,i).^alpha_hewitt*(rhow-rhoi)*g/(eta_w);
